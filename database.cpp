...
QList<QPair<QString, QString>> DatabaseManager::getObjetsByClientCin(const QString& cin) const
{
    QList<QPair<QString, QString>> resultats;
    
    QSqlDatabase& db = m_connection.getDatabase();
    if (!db.isOpen()) {
        qDebug() << "getObjetsByClientCin: Base de donnees non ouverte";
        return resultats;
    }
    
    qDebug() << "getObjetsByClientCin: Recherche ID_CLIENT pour CIN:" << cin;
    int idClient = getClientIdByCin(cin);
    qDebug() << "getObjetsByClientCin: ID_CLIENT trouve:" << idClient;
    
    if (idClient <= 0) {
        qDebug() << "getObjetsByClientCin: ID_CLIENT invalide, aucun objet retourne";
        return resultats;
    }
    
    QString tableName = detectTableName();
    QString upperName = tableName.toUpper();
    qDebug() << "getObjetsByClientCin: Table detectee:" << tableName;
    ...
    if (upperName.contains("OBJET_ELECTRONIQUE")) {
        ...
        testQuery.prepare("SELECT COUNT(*) FROM OBJET_ELECTRONIQUE WHERE ID_CLIENT_COURANT = :id_client");
        testQuery.bindValue(":id_client", idClient);
        if (testQuery.exec() && testQuery.next()) {
            int count = testQuery.value(0).toInt();
            qDebug() << "getObjetsByClientCin: Nombre d'objets avec ID_CLIENT_COURANT =" << idClient << ":" << count;
        } else {
            qDebug() << "getObjetsByClientCin: Erreur lors du test COUNT:" << testQuery.lastError().text();
        }
        
        selectQuery = "SELECT ID_OBJET, NVL(ETAT, 'Non defini') AS ETAT_OBJET "
                     "FROM OBJET_ELECTRONIQUE "
                     "WHERE ID_CLIENT_COURANT = :id_client "
                     "ORDER BY ID_OBJET";
    } else {
        selectQuery = QString("SELECT %1, %2 FROM %3 WHERE id_client_courant = :id_client ORDER BY %1")
                     .arg(getTableColumns(tableName)[0])
                     .arg(getTableColumns(tableName)[7])
                     .arg(tableName);
    }
    
    qDebug() << "getObjetsByClientCin: Requete SQL:" << selectQuery;
    qDebug() << "getObjetsByClientCin: ID_CLIENT utilise:" << idClient;
    ...
    if (!query.exec()) {
        ...
        qDebug() << "getObjetsByClientCin: Erreur SQL:" << error;
        qDebug() << "getObjetsByClientCin: Code erreur:" << query.lastError().nativeErrorCode();
        qDebug() << "getObjetsByClientCin: Database error:" << query.lastError().databaseText();
        return resultats;
    }
    
    int count = 0;
    while (query.next()) {
        QString idObjet = query.value(0).toString();
        QString etat = query.value(1).toString();
        if (etat.isEmpty() || etat.isNull()) {
            etat = "Non defini";
        }
        qDebug() << "getObjetsByClientCin: Objet trouve - ID:" << idObjet << "Etat:" << etat;
        resultats.append(qMakePair(idObjet, etat));
        count++;
    }
    
    qDebug() << "getObjetsByClientCin: Total objets trouves:" << count;
    return resultats;
}
