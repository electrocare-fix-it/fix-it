    ...
    , m_arduinoSerial(nullptr)
{
    ...
    m_arduinoSerial = new ArduinoSerial(this);
    connect(m_arduinoSerial, &ArduinoSerial::cinReceived, this, &gestionobjetelectronique::traiterCinArduino);
    connect(m_arduinoSerial, &ArduinoSerial::connectionStatusChanged, this, [this](bool connected) {
        if (connected) {
...
void gestionobjetelectronique::traiterCinArduino(const QString& cin)
{
    ...
    DatabaseManager& db = DatabaseManager::getInstance();
    bool existe = db.clientExists(cin);
    qDebug() << "CIN existe dans la base:" << existe;
    
    if (existe) {
        ui->textEdit111->setPlainText("CIN trouve: " + cin);
        qDebug() << "Recherche des objets pour CIN:" << cin;
        
        QList<QPair<QString, QString>> objets = db.getObjetsByClientCin(cin);
        qDebug() << "Nombre d'objets retournes:" << objets.size();
        
        if (objets.isEmpty()) {
            ui->textEdit222->setPlainText("Aucun objet electronique trouve pour ce CIN.\n\nVerifiez que les objets ont ID_CLIENT_COURANT rempli dans la base de donnees.");
            qDebug() << "Aucun objet trouve - verifiez ID_CLIENT_COURANT dans OBJET_ELECTRONIQUE";
        } else {
            QString resultat = "Objets electroniques pour CIN " + cin + ":\n\n";
            for (int i = 0; i < objets.size(); i++) {
                resultat += QString("• ID: %1 - Etat: %2\n").arg(objets[i].first).arg(objets[i].second);
                qDebug() << "Objet" << i+1 << ":" << objets[i].first << "-" << objets[i].second;
            }
            ui->textEdit222->setPlainText(resultat);
            qDebug() << "Objets affiches dans textEdit222:" << objets.size() << "objets";
        }
    } else {
        ui->textEdit111->setPlainText("CIN non trouve: " + cin);
        ui->textEdit222->clear();
        qDebug() << "CIN non trouve dans la base de donnees:" << cin;
    }
}
